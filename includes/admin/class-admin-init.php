...
class WC_Product_Stock_Status_Admin {
	...
	public function plugin_row_links( $plugin_meta, $plugin_file ) {
		if ( WC_PSS_FILE == $plugin_file ) {
            $plugin_meta[] = sprintf('<a href="%s">%s</a>', 'https://wordpress.org/plugins/wc-product-stock-status/', __('F.A.Q',WC_PSS_TXT) );
            $plugin_meta[] = sprintf('<a href="%s">%s</a>', 'https://github.com/technofreaky/WC-Product-Stock-Status', __('View On Github',WC_PSS_TXT) );
            $plugin_meta[] = sprintf('<a href="%s">%s</a>', 'https://wordpress.org/plugins/wc-product-stock-status/', __('Report Issue',WC_PSS_TXT) );
            $plugin_meta[] = sprintf('&hearts; <a href="%s">%s</a>', 'http://paypal.me/varunsridharan23', __('Donate',WC_PSS_TXT) );
		}
		return $plugin_meta;
	}	    
}